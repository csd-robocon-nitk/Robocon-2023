...
float pid_distance(int distance){
  if (distance - INPUT_DISTANCE >1 || distance - INPUT_DISTANCE <-1){
   //Serial.print("Error ");
...
// float set_angle(int fix_angle){
//    if(z>3 || z<-3){
//     errorw = 60 - fix_angle;
//    integralw = integralw + errorw * 0.01;
//    derivativew = (errorw - previousErrorw);
//    outputw = 60 + Kpw*errorw + Kiw*integralw + Kdw*derivativew;
//    previousErrorw  = errorw;
//   }
//   else{
//   errorw=0;
...
void loop() {
  ...
  int dist_read_front = ultrasonicfront.MeasureInCentimeters();
  int dist_read_side = ultrasonicside.MeasureInCentimeters();
  glb_vel[0] = -1*pid_distance(dist_read_front);
  glb_vel[1] = pid_distance(dist_read_side)/2;
  multiply();
  ...
  // Serial.print(dist_read_front);
  // Serial.print(dist_read_side);
  //pick(&lift, lift_sts);
  Serial.println();
}
